...
public class Symbols{
    ...
    private int correct; //This counts how many correct symbols the player presses
    ...
    public Symbols(int[]order,Rectangle mod,Image[]options){ //Order is the line of symbols that we are choosing from
        ...
        while(right.size()<4){
            for(int j=0;j<order.length;j++){ //This will have up to 10
                if(selected.indexOf(order[j])>0){//If the element we're looking at in order is in our arraylist
        ...
        for(int i=0;i<2;i++){
            for(int j=0;j<2;j++){
                ...
                for(int k=0;k<coloured;k++){
                    ArrayList<Integer>current=(ArrayList<Integer<)selected.clone();
